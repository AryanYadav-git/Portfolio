    ...
    <div className="border flex flex-col justify-between border-dark-500 p-4 rounded-xl">
      ...
      <div className="">
        <div className="flex w-full gap-4 ">
          {url && (
            <Link href={url} className="text-green-300 ">
              Website
            </Link>
          )}
          <Link href={github!} className="text-blue-300 ">
            Github
          </Link>
          {readMore && (
            <Link href={readMore} className="text-pink-300 ">
              Read More
            </Link>
          )}
        </div>
        <div className="flex flex-wrap w-full gap-1 md:gap-2 ">
          {stack!.map((icon, index) => (
            <div
              key={index}
              className="flex items-center  text-xs md:text-sm  text-cyan-300"
            >
              {icon}
            </div>
          ))}
